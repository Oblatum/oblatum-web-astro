...
export function HeaderMenu(active:{active:number}) {
    // 折叠菜单状态
    ...
    //活动菜单
    const [activeMenu,setActiveMenu] = useState(HeaderMenuConfig[active.active].name)
            ...
            <List className={`absolute left-0 top-0 w-full border-b-1 translate-y-[-115%] transition-transform transition-duration-500 transition-delay-150 transition-ease-out border-b-[var(--md-sys-color-surface-variant)] px-8 pb-8 pt-4rem shadow-xl md:hidden ${headerFoldMenuState ? 'translate-y-0':''}`}>
                {
                    HeaderMenuConfig.map((item:any) => (
                        <a key={item.name} href={`${item.path}`}>
                            <ListItem type="button" className={`transition-all transition-duration-300 ${activeMenu===item.name ? 'bg-[var(--md-sys-color-primary-container)]':''}`}
                            onClick={()=>{
            ...
            <div className="hidden md:flex items-center gap-4">
                {
                    HeaderMenuConfig.map((item) => (
                        <a key={item.name} href={`${item.path}`}>
                            <TextButton has-icon="true" className={`before:(content-[''] absolute top-0 left-0 w-full h-full block transition-all transition-duration-300) overflow-hidden 
